...
async function fetchEpgPrograms(date){
  ...
  return parseEpgTimeline(raw,isJson,date);
}
// 時刻を05:00始まりの連続tv分(300〜1739)に正規化。深夜0:00〜4:59は翌日扱い(+1440)
const clockToTv=(h,m)=>{let t=h*60+m;return t<300?t+1440:t;};
// tv分を表示用の時刻文字列(HH:MM)へ
const tvToClock=tv=>{const t=((tv%1440)+1440)%1440;return `${String(Math.floor(t/60)).padStart(2,"0")}:${String(t%60).padStart(2,"0")}`;};
...
// 実番組(epg-all)×実分析コーナー(daily_corners)を、指定slot窓(朝5:30-8:30/夕方15:30-19:00、
// または"full"指定で終日05:00-29:00)で突き合わせてtpl形式
// ({[stId]:[[progName,startHHMM,endHHMM,corners[]],...]})に組み立てる。
...
function buildDayTpl(programs,corners,date,slot){
  ...
  const slotStartAbs=dayMid+(slot==="morning"?330:slot==="evening"?930:300);
  const slotEndAbs=dayMid+(slot==="morning"?510:slot==="evening"?1140:1740);
...
function stripPtSign(text){
  ...
  // AI分析は視聴率ベースで行っており「占拠率」という語はプロンプト指示だけでは守られないことが
  // あった(実際に2回発生)ため、ここでも機械的に置換する(表示前の最終防波堤)
  out=out.replace(/占拠率/g,"視聴率");
  return out;
}
...
// 帯番組の「曜日別 週次/月次推移」グラフ。横軸は期間(週または月)、月〜金それぞれを1本の折れ線にして、
// 同じ横軸目盛りに5つの点が並ぶ形で重ね描きする(TV pop互換の見せ方)。
// periods[i]={label, values:[月,火,水,木,金]}, values[di]は null(データなし)/undefined(読込中)/
// {v,navDate,dateLabel}(値あり)のいずれか。週次・月次のどちらの意味付けかはこの呼び出し側で決めるので、
// このコンポーネント自体はモードを意識しない
function ProgramWeeklyTrendChart({periods,onSelectDate}){
  const cRef=useRef(null);
  // 初期値をnullにし、実際のコンテナ幅を測るまでSVGを描画しない。900px固定の初期値のままだと、
  // 測定が終わるまでの一瞬だけグラフが横に広がって表示されてしまう(ResizeObserverが効く前のチラつき)
  const[w,setW]=useState(null);
  const[hover,setHover]=useState(null); // {i,di,v,navDate,dateLabel}
  useEffect(()=>{const o=new ResizeObserver(es=>{for(const e of es)setW(e.contentRect.width);});if(cRef.current)o.observe(cRef.current);return()=>o.disconnect();},[]);
  const h=200,p={t:16,r:16,b:30,l:PROGRAM_LABEL_WIDTH};
  const cW=Math.max(1,(w??0)-p.l-p.r),cH=h-p.t-p.b;
  const n=periods.length;
  const allVals=periods.flatMap(pr=>pr.values).map(c=>c?.v).filter(v=>typeof v==="number");
        ...
        return <g key={lb}>
          ...
          {pts.map(pt=><g key={pt.i}>
            ...
            <circle cx={xS(pt.i)} cy={yS(pt.v)} r={9} fill="transparent" style={{cursor:onSelectDate?"pointer":"default"}}
              ...
              onClick={()=>onSelectDate&&onSelectDate(pt.navDate)}/>
          </g>)}
        </g>;
      })}
      {hover&&(()=>{
        const cx=xS(hover.i),cy=yS(hover.v);
        const boxW=Math.max(104,hover.dateLabel.length*7+16),boxH=34;
        ...
        return <g pointerEvents="none">
          ...
          <text x={bx+8} y={by+14} fontSize="10" fontFamily="monospace" fill="#374151" fontWeight="600">{hover.dateLabel}</text>
    ...
    <div style={{display:"flex",alignItems:"flex-start",minHeight:"calc(100vh - 140px)"}}>
      <div style={{flex:"1 1 0",display:"flex",flexDirection:"column",minWidth:0,overflow:"visible",padding:"16px 18px"}}>
        ...
        {progDef.weeklyTrend&&weeklySeries&&<div style={{background:"#fff",border:"1px solid #e0e0e0",borderRadius:18,padding:16,marginBottom:16}}>
          ...
          <ProgramWeeklyTrendChart periods={weeklySeries} onSelectDate={setRefDate}/>
        </div>}
...
async function buildAnalysisContext(dates,slot,ratingsCache,tplByDate){
  ...
  for(const date of dates){
    ...
    lines.push("【重要・厳守】以下のデータの指標はすべて視聴率(%)である。「占拠率」という言葉は一切使わないこと(占拠率は別の指標であり、このデータには一切含まれていない)。");
    if(slot==="morning"){
      lines.push("【重要・厳守】NHK総合の連続テレビ小説(朝ドラ)は毎日08:00に開始し08:15に終了する。08:00台の視聴者の動きについて書く際、「朝ドラ終了後」「朝ドラが終わって」のような表現は誤りであり、絶対に使わないこと。08:00は朝ドラが始まったばかりの時間帯である。08:00台の流入・流出は「朝ドラ開始に伴う動き」「朝ドラ視聴を避けた層の動き」のように書くこと。");
    }
    // 視聴質パネル調査による実測流入流出データ(inout/)があれば使う。無い日/局は404/403で単に諦め、
    // 既存の視聴率ベース推定にフォールバックする
    ...
    for(const sid of ST.map(s=>s.id)){
      ...
      for(const[progName,progStart,progEnd,corners] of progs){
        ...
        // NHKの朝ドラ(毎日08:00-08:15固定)には、AIが「終了後」と誤読しないよう直接注記する
        const asadoraNote=(sid==="NHK"&&progStart==="08:00"&&progEnd==="08:15")?"【この番組は朝ドラ。08:00は開始直後(終了は08:15)】":"";
        lines.push(`  ${sid}「${progName}」(${progStart}–${progEnd}) AVG${pavg.toFixed(1)}% 最高${ppeak.toFixed(1)}%(${ppeakRow.time}時点)${asadoraNote}${top2?` 主要:${top2}`:""}`);
      }
    }
  }
...
function AnalysisPage({page,setPage,metric,setMetric,ratingsCache,weatherData,
  ...
  const runTopicAnalysis=async()=>{
    ...
    // 局別集計（選択コーナーのみ、視聴率ベース）
    const byStation={};
    for(const c of selectedCorners){
      const stats=computeCornerStats(c,ratingsCache,"rating"); // 視聴率
    ...
    for(const sid of Object.keys(byStation)){
      ...
      for(const c of byStation[sid].corners){
        ...
        const flows=computeRivalFlow(sid,sM,eM,rData2,tplFor(c.date,c.slot));
        ...
        {ST.map(st=>{
          ...
          return <div key={st.id} style={{marginBottom:12,background:"#fff",border:"1px solid #E5E7EB",borderRadius:8,overflow:"hidden"}}>
            ...
            {progNames.map(progName=>{
              ...
              // 番組内の全コーナーの視聴率AVG平均
              const avgRating=(()=>{
                const vals=progCorners.map(c=>computeCornerStats(c,ratingsCache,"rating").avg).filter(v=>v!=null);
              ...
              return <div key={key} onClick={()=>setTopicSelected(prev=>{const n=new Set(prev);isSel?n.delete(key):n.add(key);return n;})}
                ...
                {avgRating!=null&&<span style={{fontSize:10.5,fontFamily:"monospace",color:st.c,fontWeight:700}}>視聴率AVG {avgRating.toFixed(1)}%</span>}
              </div>;
            })}
          </div>;
        })}
        ...
        {topicResult&&<>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div>
              ...
              <div style={{fontSize:10,color:"#9CA3AF",marginTop:2}}>対象: 実分析結果がある全期間 × 朝帯・夕方帯 ／ 指標: <b style={{color:"#0066cc"}}>視聴率</b></div>
            </div>
          ...
          <div style={{background:"#fff",border:"1px solid #E5E7EB",borderRadius:8,overflow:"hidden",marginBottom:16}}>
            <div style={{display:"grid",gridTemplateColumns:"54px 1fr 64px 64px 64px 64px 60px",gap:6,padding:"8px 12px",background:"#F9FAFB",borderBottom:"1px solid #E5E7EB",fontSize:9,color:"#6B7280",fontFamily:"monospace",fontWeight:700}}>
              <div>局</div><div>番組</div><div style={{textAlign:"right"}}>IN avg</div><div style={{textAlign:"right"}}>OUT avg</div><div style={{textAlign:"right"}}>視聴率AVG</div><div style={{textAlign:"right"}}>DIFF avg</div><div style={{textAlign:"right"}}>件数</div>
            </div>
...
function stripProgramWeekdays(query){
  ...
  return value.replace(/^[の\s]+|[の\s]+$/g,"").trim();
}
...
function DataDownloadPage(){
  ...
  const initialCond={date:null,stations:[],attrs:[],range:null,weekdays:[],targets:[]};
  ...
  const promptForMissing=(miss,c)=>{
    ...
    const known=[
      ...
      c.weekdays?.length?`曜日「${c.weekdays.map(d=>["日","月","火","水","木","金","土"][d]).join("・")}曜」`:null,
    ].filter(Boolean);
  ...
  const removeCondition=kind=>{
    ...
    if(kind==="weekdays"){next={...current,weekdays:[]};label="曜日";}
    commitCond(next);
    setProgMatches(null);setSelectedProgramKeys(new Set());setPendingAllChoice(null);setPendingYearChoice(null);setErr("");
  ...
  const applyProgramTargets=(programs,baseCond)=>{
    ...
    const targets=programs
      .map(p=>({date:p.date,stId:p.stId,title:p.title,startTv:p.startTv,endTv:Math.min(p.endTv,1739)}))
      .sort((a,b)=>a.date.localeCompare(b.date)||a.startTv-b.startTv||a.stId.localeCompare(b.stId));
    const targetStations=[...new Set(targets.map(t=>t.stId))];
    ...
    say("bot",`以下の${targets.length}件をダウンロードしますね。\n${list}`);
    advance(c);
  };
  ...
  const runProgramSearch=async(query,c,searchDates,{selectAll=false}={})=>{
    ...
    try{
      ...
      const fetched=await Promise.allSettled(targetDates.map(async date=>({date,progs:await fetchEpgPrograms(date)})));
      ...
      const titleHits=bestScore==null?[]:bestScore<100
        ?candidates.filter(p=>p.matchScore<100)
        :candidates.filter(p=>p.matchScore<=bestScore+4);
      const hit=titleHits.filter(p=>{
        if(c.weekdays?.length&&!c.weekdays.includes(new Date(`${p.date}T00:00:00`).getDay()))return false;
        if(c.range&&(p.startTv!==c.range.startTv||p.endTv!==c.range.endTv))return false;
        return true;
      });
      ...
      if(!hit.length){
        ...
        const filters=[
          ...
          `放送局「${stationScope}」`,
          c.weekdays?.length?`曜日「${c.weekdays.map(d=>["日","月","火","水","木","金","土"][d]).join("・")}曜」`:null,
          c.range?`放送時間「${tvToClock(c.range.startTv)}〜${tvToClock(c.range.endTv)}」`:null,
        ].filter(Boolean).join("・");
        say("bot",titleHits.length
          ?`番組名「${query}」では${titleHits.length}件見つかりましたが、${filters}をすべて満たす放送回はありませんでした。\n時間や曜日の条件を外すか、正しい放送時間を入力してください。番組名と期間は保持しています。`
          :`${filters}までは認識できています。\nただし、番組表の正式タイトルから「${query}」に一致する番組を確認できませんでした。\n\n番組名の別表記・放送局・放送時刻のうち、分かるものだけ追加してください。期間の条件は保持しています。`);
      }else if(selectAll){
        setProgMatches(hit);
        setSelectedProgramKeys(new Set(hit.map(programKey)));
        const narrowed=[c.weekdays?.length?`${c.weekdays.map(d=>["日","月","火","水","木","金","土"][d]).join("・")}曜`:null,c.range?`${tvToClock(c.range.startTv)}〜${tvToClock(c.range.endTv)}`:null].filter(Boolean);
        say("bot",`「${query}」${narrowed.length?`を${narrowed.join("・")}で絞り込み、`:"に一致する"}${hit.length}件が見つかりました。すべて選択しています。不要な放送回のチェックを外してから進んでください。`);
      }else{
        setProgMatches(hit);
        setSelectedProgramKeys(new Set());
  ...
  const resolveAllMeaning=async choice=>{
    const pending=pendingAllChoice;if(!pending)return;
    setPendingAllChoice(null);
    ...
    const searchDates=pending.dateCandidates.length?pending.dateCandidates:activeSearchDates.length?activeSearchDates:(c.date?[c.date]:[]);
    if(!searchDates.length){
      setPendingProgram(pending.programQuery);
      setFocus("date");
      say("bot","番組を検索する日付・期間を指定してください。（例:「今月」「先週末」）");
      return;
    }
    await runProgramSearch(pending.programQuery,c,searchDates,{selectAll:choice==="episodes"});
  };

  const chooseAllMeaning=choice=>{
    say("user",choice==="episodes"?"該当する放送回を全部":"属性を全部");
    resolveAllMeaning(choice);
  };
  ...
  const handleSend=async forcedText=>{
    ...
    if(p.dateCandidates.length===1){
      ...
      got.push(`検索期間=${p.dateCandidates[0]}〜${p.dateCandidates[p.dateCandidates.length-1]}${dateExpression?`（${dateExpression}）`:""}`);
    ...
    if(requestedWeekdays.length){
      c.weekdays=requestedWeekdays;
      got.push(`曜日=${requestedWeekdays.map(d=>["日","月","火","水","木","金","土"][d]).join("・")}曜`);
    }
    ...
    if(needsAllClarification){
      ...
      setPendingProgram("");
      setFocus("allMeaning");
      say("bot","確認です。「全部」はどちらの意味ですか？\n1. 条件に該当する放送回を全部\n2. 視聴率の属性を全部");
      return;
    }
    if(pendingProgram&&p.dateCandidates.length){
      const q=pendingProgram;
      setPendingProgram("");
      await runProgramSearch(q,c,p.dateCandidates,{selectAll:wantsAllMatches});
      return;
    }
    // 候補表示後に時間・曜日・局を追加した場合も、元の番組名を保持したまま再検索する。
    const hasSearchRefinement=!!p.range||requestedWeekdays.length>0||p.stations.length>0;
    const query=p.programQuery||(focus==="program"&&hasSearchRefinement?activeProgramQuery:null)||(focus==="range"?text:null);
    const wantProg=!!query&&(!!p.programQuery||focus==="range"||(focus==="program"&&hasSearchRefinement));
    if(wantProg&&query){
  ...
  const confirmDate=d=>{const c={...cond,date:d};setActiveSearchDates([d]);commitCond(c);say("user",`${d}（${DOW(d)}）`);
  ...
  const toggleProgram=p=>setSelectedProgramKeys(keys=>{const next=new Set(keys);const key=programKey(p);next.has(key)?next.delete(key):next.add(key);return next;});
  const confirmPrograms=()=>{
    const selected=(progMatches||[]).filter(p=>selectedProgramKeys.has(programKey(p)));
    if(selected.length)applyProgramTargets(selected,cond);
  };
  const confirmTime=()=>{
  ...
  const doDownload=async()=>{
    setBusy(true);setErr("");
    try{
      const{date,stations,attrs,range,targets=[]}=cond;
      const wb=XLSX.utils.book_new();
      const safe=s=>s.replace(/[\\/:*?"<>|（）]/g,"_").replace(/〜/g,"-").slice(0,40);
      const usedSheetNames=new Set();
      const sheetName=(raw,index)=>{
        const base=safe(raw).replace(/[\[\]]/g,"_").slice(0,27)||`放送回${index+1}`;
        let name=base,n=2;
        while(usedSheetNames.has(name))name=`${base.slice(0,27-String(n).length)}_${n++}`;
        usedSheetNames.add(name);
        return name;
      };
      const makeSheet=(header,rows,targetStations,targetRange)=>{
        const cols=[];
        for(const stId of targetStations)for(const a of attrs){
          const key=`${RATING_HEADER_STATION[stId]}_${a}`;
          const idx=header.indexOf(key);if(idx===-1)continue;
          cols.push({idx,label:`${ST.find(s=>s.id===stId).nm} ${RATING_ATTRS.find(x=>x.id===a).lb}`});
        }
        if(!cols.length)throw new Error("該当する列が見つかりませんでした。");
        const out=[["時刻",...cols.map(c=>c.label)]];
        for(const r of rows){
          const mm=String(r[0]).match(/^(\d{1,2}):(\d{2})/);if(!mm)continue;
          const tv=clockToTv(+mm[1],+mm[2]);
          if(tv<targetRange.startTv||tv>targetRange.endTv)continue;
        ...
        ws["!cols"]=[{wch:8},...cols.map(()=>({wch:16}))];
        return{ws,minutes:out.length-1,series:cols.length};
      };
      let fn,totalMinutes=0,totalSeries=0;
      if(targets.length){
        const summary=[["日付","曜日","放送局","番組名","開始","終了"]];
        const tableByDate=new Map();
        await Promise.all([...new Set(targets.map(t=>t.date))].map(async d=>tableByDate.set(d,await fetchRawRatingTable(d))));
        targets.forEach((t,i)=>{
          const table=tableByDate.get(t.date);
          const result=makeSheet(table.header,table.rows,[t.stId],t);
          totalMinutes+=result.minutes;totalSeries+=result.series;
          XLSX.utils.book_append_sheet(wb,result.ws,sheetName(`${t.date.slice(5).replace("-","")}_${t.title}`,i));
          summary.push([t.date,DOW(t.date),ST.find(s=>s.id===t.stId).nm,t.title,tvToClock(t.startTv),tvToClock(t.endTv)]);
        });
        const summaryWs=XLSX.utils.aoa_to_sheet(summary);
        summaryWs["!cols"]=[{wch:12},{wch:6},{wch:12},{wch:48},{wch:8},{wch:8}];
        XLSX.utils.book_append_sheet(wb,summaryWs,"対象番組一覧");
        const first=targets[0].date,last=targets[targets.length-1].date;
        fn=`視聴率_番組まとめ_${first}_${last}_${targets.length}件.xlsx`;
      }else{
        const{header,rows}=await fetchRawRatingTable(date);
        const result=makeSheet(header,rows,stations,range);
        totalMinutes=result.minutes;totalSeries=result.series;
        XLSX.utils.book_append_sheet(wb,result.ws,"視聴率");
        fn=`視聴率_${date}_${safe(range.label)}.xlsx`;
      }
      XLSX.writeFile(wb,fn);
      say("bot",targets.length
        ?`${targets.length}件の放送回を、1つのExcelにまとめてダウンロードしました 🎉\n番組ごとにシートを分けています。\nファイル名: ${fn}`
        :`Excelをダウンロードしました 🎉\n（${totalMinutes}分ぶん × ${totalSeries}系列）\nファイル名: ${fn}`);
      setFocus("done");
  ...
  return <div style={{maxWidth:820,margin:"0 auto",padding:"20px 18px 40px",display:"flex",flexDirection:"column",minHeight:"calc(100vh - var(--topbar-height))"}}>
    ...
    {/* チャットログ */}
    <div ref={logRef} style={{flex:1,overflowY:"auto",background:"#fff",border:"1px solid #e0e0e0",borderRadius:18,padding:"18px",display:"flex",flexDirection:"column",gap:12}}>
      {log.map((m,i)=><div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
        <div style={{maxWidth:"78%",whiteSpace:"pre-wrap",padding:"10px 14px",borderRadius:18,fontSize:13.5,lineHeight:1.5,letterSpacing:"-0.2px",
          background:m.role==="user"?"#0066cc":"#f5f5f7",color:m.role==="user"?"#fff":"#1d1d1f",
          borderTopRightRadius:m.role==="user"?4:18,borderTopLeftRadius:m.role==="user"?18:4}}>{m.text}</div>
      </div>)}
      {busy&&<div style={{display:"flex",justifyContent:"flex-start"}}><div style={{padding:"10px 14px",borderRadius:18,background:"#f5f5f7",fontSize:13,color:"#7a7a7a"}}>処理中…</div></div>}
    </div>
    ...
    <div style={{marginTop:14,background:"#fff",border:"1px solid #e0e0e0",borderRadius:18,padding:"16px"}}>
      ...
      {/* 現在の認識状況(サマリ) */}
      {focus!=="done"&&Boolean(cond.date||cond.stations.length||cond.attrs.length||cond.range||cond.weekdays?.length||cond.targets.length||condHistory.length)&&<div style={{display:"flex",flexWrap:"wrap",gap:6,alignItems:"center",marginBottom:14}}>
        ...
        {!cond.targets.length&&cond.weekdays?.length>0&&conditionPill(`曜日: ${cond.weekdays.map(d=>["日","月","火","水","木","金","土"][d]).join("・")}曜`,()=>removeCondition("weekdays"))}
      ...
      {focus==="allMeaning"&&<div>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          {cta(()=>chooseAllMeaning("episodes"),false,"該当する放送回を全部")}
          {ghost(()=>chooseAllMeaning("attributes"),"視聴率の属性を全部")}
        </div>
      </div>}
      ...
      {focus==="program"&&<div>
        {progMatches&&progMatches.length>0&&<div style={{display:"flex",flexDirection:"column",gap:8}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
            <div style={{fontSize:12,color:"#7a7a7a"}}>ダウンロードする放送回を選んでください（{progMatches.length}件・複数選択可）</div>
            <button onClick={()=>setSelectedProgramKeys(selectedProgramKeys.size===progMatches.length?new Set():new Set(progMatches.map(programKey)))} style={{border:"none",background:"transparent",color:"#0066cc",fontSize:12,fontWeight:600,cursor:"pointer",padding:4}}>
              {selectedProgramKeys.size===progMatches.length?"全件解除":"全件選択"}
            </button>
          </div>
          {progMatches.map((p,i)=>{const selected=selectedProgramKeys.has(programKey(p));return <button key={i} onClick={()=>toggleProgram(p)} style={{textAlign:"left",padding:"10px 14px",borderRadius:12,border:`1px solid ${selected?"#0066cc":"#e0e0e0"}`,background:selected?"#f0f7ff":"#fff",cursor:"pointer",display:"flex",alignItems:"center",gap:10}}>
            <span style={{width:18,height:18,borderRadius:5,border:`1px solid ${selected?"#0066cc":"#c7c7cc"}`,background:selected?"#0066cc":"#fff",color:"#fff",display:"grid",placeItems:"center",fontSize:12,flexShrink:0}}>{selected?"✓":""}</span>
      ...
      {focus==="confirm"&&<div>
        <div style={{background:"#f5f5f7",borderRadius:12,padding:"14px 16px",marginBottom:14,fontSize:13.5,lineHeight:1.9,color:"#1d1d1f"}}>
          {cond.targets.length>0?<>
            <div style={{fontWeight:600,marginBottom:5}}>以下の{cond.targets.length}件をダウンロードします</div>
            {cond.targets.map((t,i)=><div key={programKey(t)} style={{display:"grid",gridTemplateColumns:"24px minmax(0,1fr)",gap:4,padding:"7px 0",borderTop:i?"1px solid #e5e5e7":"none",lineHeight:1.5}}>
              <span style={{color:"#0066cc",fontWeight:700}}>{i+1}</span>
              <span><span style={{fontWeight:600}}>{t.date}（{DOW(t.date)}） {ST.find(s=>s.id===t.stId).nm}</span><br/>{t.title}<br/><span style={{color:"#7a7a7a"}}>{tvToClock(t.startTv)}〜{tvToClock(t.endTv)}</span></span>
            </div>)}
          </>:<>
...
export default function App(){
  ...
  const effectiveZoomWidths=useMemo(()=>{
    ...
    return[domainWidth,...new Set([...extra,...ZOOM_WIDTHS.filter(w=>w<domainWidth)])].sort((a,b)=>b-a);
  },[domainWidth]);
  const viewWidth=effectiveZoomWidths[Math.min(zoomLevel,effectiveZoomWidths.length-1)];
  const minCenter=domainStart+viewWidth/2,maxCenter=domainEnd-viewWidth/2;
